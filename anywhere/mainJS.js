...
var teaRounderData = {
	"Categories": {
		...
		"Speciality Teas": { 
			...
			"mainSelects": [ 
				["Green Tea", "Peppermint Tea", "Camomile Tea", "Chai Latte Tea", "Rooibos Tea", "Lemon Ginger Tea", "Redbush Tea"]
			]
		},
		"Fancy Coffees": { 
			"listingTemplate": "Default", 
			"mainSelects": [ 
				["Flat White Coffee", "Espresso", "Latte", "Cappuccino", "Caramel Macchiato", "Americano", "Cafe Mocha", "White Chocolate Mocha"]
			]
		},
		"Other Hot Drinks": { 
			"listingTemplate": "Default", 
...
*/
  ...
  document.getElementById("saveDrinksLocal").disabled = "";
...
function resetDrinks(){
    if(confirm("are you sure? This empties the entire list")){
        ...
        document.getElementById("saveDrinksLocal").disabled = "disabled";
		document.getElementById("totalQuantity").innerHTML  = 0;
    }
}

function saveDrinksLocal(){
	if (!confirm("Currently, only one list can be saved at a time. Continue?")){
		return;
	}
	if (saveToLocalStorage("currentDrinksList", document.getElementById("drinks").innerHTML)){
		alert("saved");
	}
}

function loadDrinksLocal(){
	if(localStorage.getItem("currentDrinksList") == ""){
		alert("No saved list found");
	} else {
		document.getElementById("drinks").innerHTML = localStorage.getItem("currentDrinksList");
		document.getElementById("resetDrinks").disabled = "";
		document.getElementById("saveDrinksLocal").disabled = "";
		//must also calculate totalQuantity and then set it  - drinkline
		let amountElems = document.getElementById("drinks").getElementsByClassName("Amount");
		let totalQuantity = 0;
		for (var i = 0; i < amountElems.length; i++) {
			totalQuantity += parseInt(amountElems[i].innerHTML);
		}
